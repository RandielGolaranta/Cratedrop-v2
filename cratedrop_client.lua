...
local requiredModels = {"p_cargo_chute_s", "ex_prop_adv_case_sm", "bombushka", "s_m_m_pilot_02", "prop_box_wood02a_pu"} -- parachute, pickup case, plane, pilot, crate
...
function CrateDrop(weapon, ammo, planeSpawnDistance, dropCoords)
    Citizen.CreateThread(function()
        ...
        aircraft = CreateVehicle(GetHashKey("bombushka"), rPlaneSpawn, heading, true, true)
        ...
        SetPlaneMinHeightAboveTerrain(aircraft, 20) -- the plane shouldn't dip below the defined altitude

        TaskVehicleDriveToCoord(pilot, aircraft, vector3(dropCoords.x, dropCoords.y, dropCoords.z) + vector3(0.0, 0.0, 500.0), 60.0, 0, GetHashKey("bombushka"), 262144, 15.0, -1.0) -- to the dropsite, could be replaced with a task sequence
        ...
        TaskVehicleDriveToCoord(pilot, aircraft, 0.0, 0.0, 500.0, 60.0, 0, GetHashKey("bombushka"), 262144, -1.0, -1.0) -- disposing of the plane like Rockstar does, send it to 0; 0 coords with -1.0 stop range, so the plane won't be able to achieve its task
        ...
        SetBlipNameFromTextFile(blip, "CAJA SUMINISTROS")
        ...
        TriggerServerEvent('cratedrop:loot')
...
local coordinadas = vector3(1229.1,3067.31, 50.87)  //places where you can get a drop
local coordinadas1 = vector3(-1227.6,-1373.35, 3.12)
local coordinadas2 = vector3(178.17,-826.44, 30.17)
local coordinadas3 = vector3(-1335.89,4471.18, 22.1)
local size = 2.0
local height = 2.0

RegisterNetEvent('cooldown:suministro')
AddEventHandler('cooldown:suministro', function()
...
Citizen.CreateThread(function()
    cooldownGeamuriTime = 1800       -- cooldown time in secs for each drop 
    cooldownGeamuri = false     -- no modificar
    while true do
        Citizen.Wait(0)
      if cooldownGeamuri == false then
        DrawMarker(7, coordinadas.x, coordinadas.y, coordinadas.z+1, 0, 0, 0, 0, 0, 0, size, size, height, 0, 155, 255, 155, 0, 0, 2, 0, 0, 0, 0)
        DrawMarker(7, coordinadas1.x, coordinadas1.y, coordinadas1.z+1, 0, 0, 0, 0, 0, 0, size, size, height, 0, 155, 255, 155, 0, 0, 2, 0, 0, 0, 0)
        DrawMarker(7, coordinadas2.x, coordinadas2.y, coordinadas2.z+1, 0, 0, 0, 0, 0, 0, size, size, height, 0, 155, 255, 155, 0, 0, 2, 0, 0, 0, 0)
        DrawMarker(7, coordinadas3.x, coordinadas3.y, coordinadas3.z+1, 0, 0, 0, 0, 0, 0, size, size, height, 0, 155, 255, 155, 0, 0, 2, 0, 0, 0, 0)
        ------INDUSTRIA location
        if  GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), coordinadas.x, coordinadas.y, coordinadas.z, true) < size  then
            helpMessage("Pulsa ~INPUT_CONTEXT~ para marcar la posicion!")   //call the drop plane
            
            if (IsControlJustPressed(1, 38)) and cooldownGeamuri == false then
                ShootSingleBulletBetweenCoords(coordinadas, coordinadas - vector3(0.0001, 0.0001, 0.0001), 0, false, GetHashKey("weapon_flare"), 0, true, false, -1.0)
                ExecuteCommand("cratedrop pistol 40 true 2000.0")   --this drop give you a pistol and loot from server.lua randomly
                TriggerEvent('cooldown:suministro')
                
            else
                if (IsControlJustPressed(1, 38)) and cooldownGeamuri == true then
                    TriggerEvent('chatMessage', '', {255, 255, 255}, "Tendras que esperar hasta el siguiente suministro en "..timeRemaining.." segundos.")  --wait till next drop
                end
                
            end
        end
        -------VENICE BEACH location
        if  GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), coordinadas1.x, coordinadas1.y, coordinadas1.z, true) < size  then
            helpMessage("Pulsa ~INPUT_CONTEXT~ para marcar la posicion!")
            
            if (IsControlJustPressed(1, 38)) and cooldownGeamuri == false then
                ShootSingleBulletBetweenCoords(coordinadas1, coordinadas1 - vector3(0.0001, 0.0001, 0.0001), 0, false, GetHashKey("weapon_flare"), 0, true, false, -1.0)
                ExecuteCommand("cratedrop pistol50 30 true 2000.0")
                TriggerEvent('cooldown:suministro')
                
            else
                if (IsControlJustPressed(1, 38)) and cooldownGeamuri == true then
                    TriggerEvent('chatMessage', '', {255, 255, 255}, "Tendras que esperar hasta el siguiente suministro en "..timeRemaining.." segundos.")
                end
                
            end
        end
        -------CIUDAD CENTRAL location in city
        if  GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), coordinadas2.x, coordinadas2.y, coordinadas2.z, true) < size  then
            helpMessage("Pulsa ~INPUT_CONTEXT~ para marcar la posicion!")
            
            if (IsControlJustPressed(1, 38)) and cooldownGeamuri == false then
                ShootSingleBulletBetweenCoords(coordinadas2, coordinadas2 - vector3(0.0001, 0.0001, 0.0001), 0, false, GetHashKey("weapon_flare"), 0, true, false, -1.0)
                ExecuteCommand("cratedrop smg 60 true 2000.0")
                TriggerEvent('cooldown:suministro')
        ...
        -------VALLE/RIO  location
        if  GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), coordinadas3.x, coordinadas3.y, coordinadas3.z, true) < size  then
            helpMessage("Pulsa ~INPUT_CONTEXT~ para marcar la posicion!")
            
            if (IsControlJustPressed(1, 38)) and cooldownGeamuri == false then
                ShootSingleBulletBetweenCoords(coordinadas3, coordinadas3 - vector3(0.0001, 0.0001, 0.0001), 0, false, GetHashKey("weapon_flare"), 0, true, false, -1.0)
                ExecuteCommand("cratedrop heavyshotgun 25 true 2000.0")
                TriggerEvent('cooldown:suministro')
